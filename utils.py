...
def plot_dispatch_week_storage(network, week1_start, week2_start, title1, title2):
    """Plot two weeks of dispatch with storage and H2 flows, stacked vertically."""
    ...
    def add_week(row, week_start, week_end):
        dispatch = _get_generator_dispatch(network).loc[week_start:week_end - pd.Timedelta(hours=1)]
    ...
    add_week(1, week1_start, week1_end)
    add_week(2, week2_start, week2_end)
...
def plot_storage_soc(network, title="Storage State of Charge – Full Year"):
    """Plot annual state of charge (%) for battery and pumped hydro in separate subplots."""
    if network.storage_units.empty:
        return
    ...
    fig = make_subplots(
        rows=2, cols=1,
        subplot_titles=["Battery", "Pumped hydro"],
    )
    ...
    for storage_name, row in storage_panels:
        if storage_name not in soc.columns:
            continue

        max_energy = p_nom_opt[storage_name] * network.storage_units.loc[storage_name, "max_hours"]
        if max_energy < 1:
            continue  # skip zero-investment storage
        soc_pct = soc[storage_name] / max_energy * 100
        color = color_map.get(storage_name, None)
